...
func GenerateShortImageDownloadURLByToken(baseUrl, sourceId, imageTokenKey string) (string, *strfmt.DateTime, error) {
	token, err := JWTForSymmetricKey([]byte(imageTokenKey), ImageExpirationTime, sourceId)
	if err != nil {
		return "", nil, errors.Wrap(err, "failed to sign image URL")
	}

	exp, err := ParseExpirationFromToken(token)
	if err != nil {
		return "", nil, err
	}

	path := fmt.Sprintf("%s/%s/discovery.ova", "/api/v1/image/bytoken/", token)
